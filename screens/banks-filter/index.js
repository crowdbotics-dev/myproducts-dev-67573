...
const BanksFilter = () => {
  ...
  return <View style={styles.container}>
      <ScrollView style={styles.scrollView}>
        <Input text="Search" value={bankName} onChange={setBankName} icon={require("./assets/searchIcon.png")} containerStyle={styles.inputContainer} />
        <TabView tabTitles={["Choose Bank"]} selected={0} />
        <View style={styles.banksList}>
          {banks.map((bank, index) => <Bank key={index} bank={bank} selected={selectedBanks.includes(bank)} onPress={() => handleBankPress(bank)} />)}
        </View>
...
const Button = params => {
  ...
  return <View style={[buttonStyles.btnContainer, params.stickToBottom ? pinned : null]}>
